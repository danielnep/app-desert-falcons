...
function load() {
  ...
  onValue(matchRef, (snap) => {
    ...
    if (state.match.status === "live" && !timerId) {
      startTimer();
    }
...
function updateMarkPanel() {
  ...
  if (hasMap()) {
    ...
    if (hint) {
      hint.textContent =
        "Escolha um arquivo de imagem (PNG, JPEG…). Depois delimite a área útil.";
    }
  }
}

  ...
  if (m.status === "live" && state.player.presence) {
    ...
    if (m.objective === "bomba") {
      ...
      if (m.bomb.exploded) {
        ...
        $("#bombStatusText").textContent = "AGUARDANDO PLANT";
...
function fillOrg() {
  ...
  if ($("#oBriefing")) $("#oBriefing").value = m.briefing || ""; \](".obj-btn:not(:disabled)").forEach(b => {
...
document.addEventListener("click", (e) => {
  ...
  if (obj && obj.dataset.obj) {
    state.match.objective = obj.dataset.obj; \](".obj-btn:not(:disabled)").forEach(b => b.classList.toggle("active", b === obj));
    saveMatchData();
...
function placeCropBox() {
  const box = $("#cropBox"), stage = $("#cropStage");
  if (!box || !stage) return;
  const r = stage.getBoundingClientRect();
...
$("#devReset")?.addEventListener("click", () => {
  ...
  localStorage.removeItem("df_player_id");
  remove(matchRef);
  remove(playersRef);
  state = initial();
  stopTimer();
